...
function handleReceivedPost() {
  ...
  var post = removeUnneededMetadata(action.data);
  var channelId = post.channel_id;

  var nextPosts = _objectSpread({}, posts, _defineProperty({}, post.id, post));
  ...
  var nextPostsInChannel = postsInChannel; // Only change postsInChannel if the order of the posts needs to change
  ...
  if (!postsInChannel[channelId].includes(post.id)) {
    ...
    nextPostsInChannel = _objectSpread({}, postsInChannel);
  ...
  if (post.root_id && (!postsInThread[post.root_id] || !postsInThread[post.root_id].includes(post.id))) {
    ...
    nextPostsInThread = _objectSpread({}, postsInThread);
  ...
  if (nextPosts[post.pending_post_id] && post.id !== post.pending_post_id) {
    ...
    var channelIndex = nextPostsInChannel[channelId].indexOf(post.pending_post_id);
    ...
    if (post.root_id && postsInThread[post.root_id]) {
      var threadIndex = nextPostsInThread[post.root_id].indexOf(post.pending_post_id);
  ...
  return {
    posts: withCombineSystemPosts.nextPosts,
...
function handleReceivedPosts() {
  ...
  var skipAddToChannel = action.skipAddToChannel; // Change the state only if we have new posts,
  ...
  if (!Object.keys(newPosts).length) {
    if (!postsInChannel[channelId]) {
      ...
      return {
        posts: posts,
        postsInThread: postsInThread,
        postsInChannel: _objectSpread({}, postsInChannel, _defineProperty({}, channelId, []))
      };
    }
  ...
  var nextPostsInChannel = _objectSpread({}, postsInChannel);

  var nextPostsInThread = _objectSpread({}, postsInThread);

  var postsForChannel = postsInChannel[channelId] ? _toConsumableArray(postsInChannel[channelId]) : [];

  var _arr = Object.values(newPosts);

  for (var _i = 0; _i < _arr.length; _i++) {
    var post = _arr[_i];
    var newPost = removeUnneededMetadata(post);

    if (newPost.delete_at > 0) {
      // Mark the post as deleted if we have it
      if (nextPosts[newPost.id]) {
        nextPosts[newPost.id] = _objectSpread({}, newPost, {
    ...
    var postsForThread = nextPostsInThread[newPost.root_id] ? _toConsumableArray(nextPostsInThread[newPost.root_id]) : [];
    ...
    case _action_types.PostTypes.RECEIVED_NEW_POST:
      {
        ...
        var nextPendingPostIds = _toConsumableArray(pendingPostIds);
    ...
    case _action_types.PostTypes.RECEIVED_POSTS:
      {
        ...
        var _nextPendingPostIds2 = _toConsumableArray(pendingPostIds);

        if (!Object.keys(newPosts).length) {
          return pendingPostIds;
        }

        var _arr2 = Object.values(newPosts);

        for (var _i2 = 0; _i2 < _arr2.length; _i2++) {
          var newPost = _arr2[_i2];
    ...
    case _action_types.PostTypes.RECEIVED_NEW_POST:
      {
        ...
        return _toConsumableArray(sendingPostIds).concat([sendingPostId]);
      }
...
function handlePostsFromSearch() {
  ...
  for (var _i3 = 0; _i3 < postIds.length; _i3++) {
    var id = postIds[_i3];
  ...
  postsForChannel.forEach(function (postList, channelId) {
    info = handleReceivedPosts(info.posts, postsInChannel, postsInThread, {
      channelId: channelId,
      data: {
        posts: postList
      }
...
function handlePostDeleted() {
  ...
  var nextPostsForThread = postsInThread; // We only need to do something if already have the post

  if (posts[post.id]) {
    nextPosts = _objectSpread({}, posts);
    nextPostsForChannel = _objectSpread({}, postsInChannel); // Mark the post as deleted

    nextPosts[post.id] = _objectSpread({}, posts[post.id], {
      state: _constants.Posts.POST_DELETED,
      file_ids: [],
      has_reactions: false
    }); // Remove any of its comments

    var channelPosts = postsInChannel[channelId] ? _toConsumableArray(postsInChannel[channelId]) : [];

    var postsForChannel = _toConsumableArray(channelPosts); // make sure we don't modify the array we loop over
    ...
    try {
      for (var _iterator = channelPosts[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var id = _step.value;
    ...
    if (postsInThread[post.id]) {
      nextPostsForThread = _objectSpread({}, postsInThread);
  ...
  return {
    posts: nextPosts,
    postsInChannel: nextPostsForChannel,
    postsInThread: nextPostsForThread
  };
}
...
function handleChannelDeleted() {
  ...
  var postsInChannel = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var postsInThread = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  var channelId = arguments.length > 3 ? arguments[3] : undefined;

  var nextPosts = _objectSpread({}, posts);

  var nextPostsForChannel = _objectSpread({}, postsInChannel);

  var nextPostsForThread = _objectSpread({}, postsInThread);
  ...
  var _loop = function _loop(postId) {
    if (nextPosts[postId].channel_id === channelId) {
      ...
      var _arr3 = Object.keys(nextPostsForThread);

      for (var _i4 = 0; _i4 < _arr3.length; _i4++) {
        var rootId = _arr3[_i4];
        nextPostsForThread[rootId] = nextPostsForThread[rootId].filter(function (id) {
          return id !== postId;
        });
...
function handleRemovePost() {
  ...
  var nextPosts = posts;
  var nextPostsForChannel = postsInChannel;
  var nextPostsForThread; // We only need to do something if already have the post

  if (nextPosts[post.id]) {
    nextPosts = _objectSpread({}, posts);
    nextPostsForChannel = _objectSpread({}, postsInChannel);
    var channelPosts = postsInChannel[channelId] ? _toConsumableArray(postsInChannel[channelId]) : []; // Remove the post itself
    ...
    var postsForChannel = _toConsumableArray(channelPosts); // Remove any of its comments
    ...
    try {
      for (var _iterator2 = channelPosts[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        var id = _step2.value;
    ...
    if (postsInThread[post.id]) {
      nextPostsForThread = nextPostsForThread || _objectSpread({}, postsInThread);
      Reflect.deleteProperty(nextPostsForThread, post.id);
    }
    ...
    if (postsInThread[post.root_id]) {
      ...
      var threadPosts = _toConsumableArray(postsInThread[post.root_id]);
  ...
  return {
    posts: nextPosts,
    postsInChannel: nextPostsForChannel,
    postsInThread: nextPostsForThread || postsInThread
  };
}
    ...
    case _action_types.PostTypes.REMOVE_POST:
      {
        ...
        if (_post && state[_post.id]) {
          var nextState = _objectSpread({}, state);

          Reflect.deleteProperty(nextState, _post.id);
          return nextState;
        }
...
function storeReactionsForPost(state, post) {
  ...
  if (post.metadata.reactions && post.metadata.reactions.length > 0) {
    ...
    try {
      for (var _iterator3 = post.metadata.reactions[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
    ...
    case _action_types.PostTypes.MOVE_HISTORY_INDEX_BACK:
      {
        ...
        var _nextIndex2 = state.index ? _objectSpread({}, state.index) : _index2;
    ...
    case _action_types.PostTypes.MOVE_HISTORY_INDEX_FORWARD:
      {
        ...
        var _nextIndex3 = state.index ? _objectSpread({}, state.index) : _index3;
